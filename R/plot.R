...
plotTiles <- function(tiles, white.density = 0.5, seed = 1) {
    `%>%` <- magrittr::`%>%`
    set.seed(seed)
    # ------ If less than 2 tiles, abort 
    if (length(tiles) < 2) {
        msg_warning("Only 1 tile. Cannot plot art. Aborting now.") %>% stop()
    }
    # ------ Get plot df
    df <- tiles %>% 
        do.call(rbind, .) %>%
        ...
        dplyr::mutate(
            ...
            fill = sample(2:4, dplyr::n(), replace = TRUE), 
